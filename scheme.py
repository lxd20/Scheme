...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        def bind_value(formals, vals):
            if(formals is nil and vals is nil):
                return
            child.define(formals.first, vals.first)
            bind_value(formals.second, vals.second)
        bind_value(formals, vals)
        # END Question 10
        return child
...
def do_and_form(expressions, env):
    ...
    if expressions.second is nil:
        n = scheme_eval(expressions.first, env, True)
        if scheme_false(n):
            return False
        else:
            return n
    else:
        n = scheme_eval(expressions.first, env)
        if scheme_false(n):
            return False
        else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    n = scheme_eval(expressions.first, env)
    if scheme_true(n) and expressions.second is nil:
        return scheme_eval(expressions.first, env, True)
    if scheme_true(n):
        return n
...
def make_let_frame(bindings, env):
    ...
    evals = scheme_eval(bindings.first.second.first, env)
    values = Pair(evals, nil)
    formals, values = extract_values(bindings.second, formals, values, env)
...
def extract_values(bindings, formals, values, env):
    if bindings is nil:
        return formals, values
    check_form(bindings.first, 2, 2)
    formals_temp = Pair(bindings.first.first, formals)
    evals = scheme_eval(bindings.first.second.first, env)
    values_temp = Pair(evals, values)
    return extract_values(bindings.second, formals_temp, values_temp, env)
...
def check_formals(formals):
    ...
    def check(formals):
        if formals is nil:
            ...
            raise SchemeError
        elif formals.first in args:
            raise SchemeError
        else:
            args.append(formals.first)
            check(formals.second)
